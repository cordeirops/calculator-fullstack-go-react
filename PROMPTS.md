...
## 13. Keypad borders, tooltips, and a custom favicon

**Prompt:** In the light theme, the number keys still had no visible
border (a screenshot was attached showing them blending into the white
card); also asked for hover tooltips on more buttons, matching the
ones power/square-root already had.

**What was produced:** Every keypad button now gets `border: 1px solid
var(--border)` (previously only the accent/function-colored keys had
enough background contrast to read as separate elements in light mode
— digit keys used the same color as the card background). Added
`title` tooltips to C, ÷, ×, −, +, %, =, and the history toggle/close
buttons, so every non-digit control explains itself on hover.

**Prompt:** Shared a 4-variant reference image generated by Gemini
("Nano Banana") — a purple equals-sign glyph on light/dark rounded-
square backgrounds — asking whether it needed further processing
before use as a favicon.

**What was produced:** Confirmed it did: the source was a 2048×2048
grid of 4 icon variants (not a favicon-ready file), and the chosen
dark variant had soft gradients and asymmetric corner rounding that
would have degraded badly at 16-32px. Rather than crop and downsample
the AI output, the icon was redrawn flat in ImageMagick using the
sampled colors snapped to the app's own `--card-bg`/`--accent`
dark-theme values (`#1c1d24` background, `#a78bfa` bars), so it stays
crisp at every size and matches the app exactly. Shipped as
`favicon.ico` (16/32/48), standalone 16×16/32×32 PNGs, and a 180×180
apple-touch-icon, replacing the unused default Vite SVG.
